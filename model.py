...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Add pose correction parameters
        self.pose_correction_threshold = 0.1  # Threshold for pose correction
        self.map_fusion_threshold = 0.8  # Threshold for map fusion
        self.last_pose = None
        self.last_map = None
    ...
    def forward(self, obs, pose_obs, maps_last, poses_last, eve_angle):
        ...
        if self.last_pose is not None:
            pose_diff = torch.norm(pose_obs - self.last_pose)
            if pose_diff < self.pose_correction_threshold:
                pose_obs = self.last_pose

        # Add map fusion
        if self.last_map is not None:
            map_overlap = torch.sum(maps_last * self.last_map) / torch.sum(maps_last)
            if map_overlap > self.map_fusion_threshold:
                maps_last = torch.max(maps_last, self.last_map)
        ...
        # 計算有新障礙物資訊的區域（translated 非零處）
        # obs_mask = (translated[:, 0:1, :, :] > 0.5).float()
        # 如果正在旋轉視角，不做障礙更新
        obs_mask = torch.zeros_like(translated[:, 0:1, :, :])
        for i in range(eve_angle.shape[0]):
            if eve_angle[i] == 0:
                obs_mask[i] = (translated[i, 0:1, :, :] > 0.5).float()

        alpha = 0.8  # 保留舊地圖的比重
        # beta = 0.95  # 未觀測區域 decay
        # # 完全禁止旋轉時更新障礙物
        # for i in range(eve_angle.shape[0]):
        #     if eve_angle[i] != 0:
        #         translated[i, 0:1, :, :] = maps_last[i, 0:1, :, :]  # 使用舊值，不用新障礙物

        # # 1. 融合觀測到的區域：使用滑動平均替代 max
        # map_pred[:, 0:1, :, :] = torch.where(
        #     obs_mask.bool(),
        #     alpha * maps_last[:, 0:1, :, :] + (1 - alpha) * translated[:, 0:1, :, :],
        #     maps_last[:, 0:1, :, :]  # 先保留原值
        # )
        # 若旋轉，保留上一幀障礙地圖
        for i in range(eve_angle.shape[0]):
            if eve_angle[i] != 0:
                map_pred[i, 0:1, :, :] = maps_last[i, 0:1, :, :].clone()
            else:
                map_pred[i, 0:1, :, :] = torch.where(
                    obs_mask[i].bool(),
                    alpha * maps_last[i, 0:1, :, :] + (1 - alpha) * translated[i, 0:1, :, :],
                    maps_last[i, 0:1, :, :]
                )


        # 2. 對沒有觀測但值偏高的歷史殘留進行 decay
        # decay_mask = (maps_last[:, 0:1, :, :] > 0.2) & (obs_mask == 0)
        # map_pred[:, 0:1, :, :][decay_mask] *= beta

        # 3. 最後 clamp 保險
        map_pred[:, 0:1, :, :] = torch.clamp(map_pred[:, 0:1, :, :], 0.0, 1.0)
        # map_pred[:, 0:1, :, :] = (map_pred[:, 0:1, :, :] > 0.5).float()

        # 其他通道依然使用 max 更新
        map_pred[:, 1:, :, :] = torch.max(maps_last[:, 1:, :, :], translated[:, 1:, :, :])
        ...
        # Update last pose and map
        self.last_pose = pose_obs.clone()
        self.last_map = maps_last.clone()

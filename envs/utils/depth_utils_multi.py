...
def splat_att_feat_nd(init_grid, feat, coords,num_atts):
    """
    Args:
        init_grid: B X nF X W X H X D X ..
        feat: B X nF X nPt
    ...
    n_dims = len(grid_dims)
    # pdb.set_trace()
    grid_flat = init_grid[:,:-num_atts,...].view(B, F-num_atts, -1)
    grid_flat_att = init_grid[:,-num_atts:,...].view(B,num_atts, -1)
    ...
    for ix_d in itertools.product(*l_ix):
        ...
        index = index.long()
        grid_flat.scatter_add_(2, index.expand(-1, F-num_atts, -1), feat[:,:-num_atts,:] * wts)
        grid_flat = torch.round(grid_flat)

        current_values = grid_flat_att.gather(2, index.expand(-1, num_atts, -1))
        new_val = torch.max(current_values,  (feat[:,-num_atts:,:] * wts))
        grid_flat_att.scatter_(2, index.expand(-1, num_atts, -1), new_val)
